...
class Gui:
				...
				for x in self.kanaEngine.randomAnswers(self.param.val('list_size')):
					...
					if self.param.val('transcription_system') != "hepburn":
						x = kanaengine.HepburnToOtherSysConvert(x,
							self.param.val('transcription_system'))
					if x[-2:] == "-2": x = x[:-2]
					self.answerButt[i] = gtk.Button("%s: %s" % (str(i + 1), x.upper()))
					self.answerButt[i].connect("clicked", self.check_answer)
	...
	def options(self, oldbox):
		...
		def portions_popup(widget, kanaset):
			...
			for i in range(len(set)):
				...
				check.connect("toggled", new_value, i)
				if temp_list[i] == 1:
		...
		option1.set_active(opt_conv["boolean"][self.param.val(
			'basic_hiragana')])
		...
		option2.set_active(opt_conv["boolean"][self.param.val(
			'modified_hiragana')])
		...
		option5.set_active(opt_conv["boolean"][self.param.val(
			'modified_katakana')])
		...
		option6.set_active(opt_conv["boolean"][self.param.val(
			'contracted_katakana')])
		...
		option7.set_active(opt_conv["boolean"][self.param.val(
			'additional_katakana')])
		...
		option9.set_active(opt_conv["answer_mode"][self.param.val(
			'answer_mode')])
		...
		adjustment = gtk.Adjustment(float(self.param.val('length')), 1, 200, 1,
			10)
	...
	def set_kana_image(self,kind,kana):
		"""Updating kana image."""
		self.kanaImage.set_from_file(os.path.join(self.datarootpath, "img",
			"kana", "%s_%s.gif" % (("k", "h")[kind], kana)))
